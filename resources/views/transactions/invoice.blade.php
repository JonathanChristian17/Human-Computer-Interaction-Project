...
<body>
    <div class="container">
        <div class="header">
            <img src="{{ public_path('favicon.ico') }}" alt="Cahaya Resort Logo" style="height: 60px; margin-bottom: 10px;">
            <div class="logo">Cahaya Resort Pangururan</div>
            <div>Invoice #{{ $transaction->order_id }}</div>
        ...
        <div class="payment-info">
            ...
            <p><strong>Payment Method:</strong> {{ ucfirst($transaction->payment_type) }}</p>
            <p><strong>Transaction ID:</strong> {{ $transaction->transaction_id }}</p>
            <p><strong>Payment Date:</strong> {{ $transaction->updated_at->format('F d, Y H:i:s') }}</p>
        </div>
